...
@section('title', translation('Login'))

@section('content')
    <form id="login-form" class="auth-form" action="{{ route('auth.login') }}" method="POST">
        @csrf
        ...
        <label class="mt-4">
            ...
            <div class="iconed-input">
                <input :type="showPassword ? 'text' : 'password'" name="password" value="{{ old('password') }}" placeholder="***************" required />
                <button type="button" class="icon" @click="togglePassword()">
